...
// Have to use async function because we are interacting with the database
app.get('/',async (request, response)=>{
    // Storing data from the database in these variables
    const todoItems = await db.collection('todos').find().toArray()
    const itemsLeft = await db.collection('todos').countDocuments({completed: false})
    // Rendering the appropriate EJS view template and passing the values into the EJS template
